...
def denormalize_continuous(values: list[float]) -> dict:
    ...
    for i, name in enumerate(FLAT_CONTINUOUS):
        ...
        v = lo + values[i] * (hi - lo)
        if name == "repetitions":
            result[name] = int(round(v))
        else:
            result[name] = round(v, 3)
    return result
...
def denormalize_layer(values: list[float]) -> dict:
    ...
    for i, name in enumerate(LAYER_CONTINUOUS):
        ...
        v = lo + values[i] * (hi - lo)
        if name == "position.x":
            layer["position"]["x"] = round(v, 2)
        elif name == "position.y":
            layer["position"]["y"] = round(v, 2)
        elif name == "scale.x":
            layer["scale"]["x"] = round(v, 2)
        elif name == "scale.y":
            layer["scale"]["y"] = round(v, 2)
        elif name == "rotation":
            layer["rotation"] = round(v, 3)
    return layer


def reconstruct_params(continuous, categorical, boolean, layer_count=None, layer_params=None) -> dict:
    ...
    if isinstance(boolean, torch.Tensor):
        params["positionCoupled"] = boolean[0].item() > 0.5
    else:
        params["positionCoupled"] = boolean > 0.5
    ...
    if layer_count is not None and layer_params is not None:
        if isinstance(layer_count, torch.Tensor):
            n_layers = layer_count.argmax().item()
        else:
